...
function render(journal) {
  ...
  function next(chapter, page) {
    var delay = DELAY.PAGE;
    ...
    if (journal[chapter].length === page) {
      // turn the chapter
      delay = DELAY.CHAPTER;
      chapter += 1;
      page = 0;
    ...
    // Out of chapters?
    if (journal.length === chapter) {
      log.push('I\'ve been defeated.');
      write(log);
      return;
    }

    log.push(journal[chapter][page]);
    write(log);
    page += 1;

    setTimeout(next.bind(0, chapter, page), delay);
  }
