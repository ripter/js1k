function createSVG(polygons) {
  var index = 0;
  //
  // DEAD END: This is already a lot larger than the code it was intended to replacea.
  //         : It still needs to loop over the polygons array and add the events.
  //
  const polys = '<polygon points="" style="" onMousedown="" onMousemove="" onMouseup="" onMouseout=""/>'.replace(/""/g, function() {
    const value = polygons[0][index] || 'ROSE';
    index++;
    return '"'+value+'"';
  });
