...
window.m = function magic(evt) {
  ...
  switch (evt.type) {
    ...
    case 'mousemove':
      if (!active) { return; }
      evt.preventDefault();
      active.deltaX = evt.pageX - active.start[0];
      active.deltaY = evt.pageY - active.start[1];
...
// ['mousedown','mousemove','mouseup', 'touchstart', 'touchend', 'touchmove'].forEach(function(eventName) {
//   elmSvg.addEventListener(eventName, window.m);
// });
// ['mousedown','mousemove','mouseup', 'touchstart', 'touchend', 'touchmove'].map(function(eventName) {
//   elmSvg.addEventListener(eventName, window.m);
// });
